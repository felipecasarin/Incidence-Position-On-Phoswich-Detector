...
def Ytable(xhole,yhole,xoff=0.,yoff=0.,istonorm=1,a=np.ones((4,4))): # theoretical yields calculated using the hole position (xhole,yhole)
	...
	YT[1][0:4]=IM.YPattern(Ypm,0,istonorm) #lines
	YT[0][0:4]=IM.YPattern(Ypm,1,istonorm) #columns  
	#print('break') 
	print(YT)       
	return YT
...
def fitpatxy(): # pattern fit of x,y
    ...
    xguess=np.sum(ruler*newExN[1][0:4])*3.
    ...
    yguess=np.sum(ruler*newExN[0][0:4])*3.
    ...
    for entryNum in range(0, len(data)):    
       ...
       # L1, L2, L3, L4, C1, C2, C3, C4 = current_data[['l1', 'l2', 'l3', 'l4', 'c1','c2', 'c3', 'c4']].values.T.tolist()
        ...
        if L1>0 and L2>0 and L3>0 and L4>0 and C1>0 and C2>0 and C3>0 and C4>0:
            ...
            new_row = {'L1':L1,'L2':L2,'L3':L3,'L4':L4,'C1':C1,'C2':C2,'C3':C3,'C4':C4,'cL1':newExN[0][0],'cL2':newExN[0][1],'cL3':newExN[0][2],'cL4':newExN[0][3],'cC1':newExN[1][0],'cC2':newExN[1][1],'cC3':newExN[1][2],'cC4':newExN[1][3],'X':result.x[1],'Y':result.x[0]}
            response = response.append(new_row, ignore_index=True)
    print("It is done.")
    response.to_excel('output_pha_6_sample.xlsx', index=False)
