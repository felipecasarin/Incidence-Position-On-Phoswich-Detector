...
data_address = 'C:/Users/Usuario/Desktop/Incidence-Position-On-Phoswich-Detector/collection_files_csv_filtered/4_2.csv'
...
def fpatxy(X,Y_dat): 
	x=X[0]
	y=X[1]
	f=ft.PatternErrorOffset(x,y,0.,0.,Y_dat)   #<====PatternErrorOffset??
	return f
...
def pos0(): 
    ...
    for entryNum in range(0, len(data)):    
        ...
        if L1>0 and L2>0 and L3>0 and L4>0 and C1>0 and C2>0 and C3>0 and C4>0:
            ...
            xypat = fitpatxy()
            new_row = {'L1':L1,'L2':L2,'L3':L3,'L4':L4,'C1':C1,'C2':C2,'C3':C3,'C4':C4,'cL1':newExN[0][3],'cL2':newExN[0][2],'cL3':newExN[0][1],'cL4':newExN[0][0],'cC1':newExN[1][0],'cC2':newExN[1][1],'cC3':newExN[1][2],'cC4':newExN[1][3],'X':result.x[0],'Y':result.x[1]}
            response = response.append(new_row, ignore_index=True)
    print("It is done.")
    response.to_excel('4_2_20_07_inv.xlsx', index=False)

def pos(x,y): 

    data_address = 'C:/Users/Usuario/Desktop/Incidence-Position-On-Phoswich-Detector/collection_files_csv_filtered/' + str(x) +'_' + str(y) +'.csv'
    data=pd.read_csv(data_address)
    ...
    for entryNum in range(0, len(data)):    
        ...
        if L1>0 and L2>0 and L3>0 and L4>0 and C1>0 and C2>0 and C3>0 and C4>0:
            ...
            response = response.append(new_row, ignore_index=True)
    print("It is done.")
    response.to_excel('outfiles_pos_calc/' + str(x) + '_' + str(y) + '_pos' + '.xlsx', index=False)
...
def pos_all():
    directory = os.fsencode('C:/Users/Usuario/Desktop/Incidence-Position-On-Phoswich-Detector/row_line_inversion_pha_6/')
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if not filename.endswith('.xlsx'):
            print(f"Skipping {filename}: Not a .xlsx file.")
            continue
        data_address = 'C:/Users/Usuario/Desktop/Incidence-Position-On-Phoswich-Detector/row_line_inversion_pha_6/' + filename
        data=pd.read_excel(data_address)
        ...
        #L1, L2, L3, L4, C1, C2, C3, C4 = current_data[['L1', 'L2', 'L3', 'L4', 'C1','C2', 'C3', 'C4']].values.T.tolist()
            ...
            if L1>0 and L2>0 and L3>0 and L4>0 and C1>0 and C2>0 and C3>0 and C4>0:
                ...
                response = response.append(new_row, ignore_index=True)
        print("It is done.")
        response.to_excel('C:/Users/Usuario/Desktop/Incidence-Position-On-Phoswich-Detector/row_line_inversion_pha_6/outfiles/output_'+ filename, index=False)
